                ...
                <div className={`grid grid-cols-1 ${isInfoCollapsed ? 'gap-0' : 'lg:grid-cols-4 gap-6'} animate-in fade-in slide-in-from-bottom-4 duration-500 transition-all`}>
                    ...
                    <div className={`${isInfoCollapsed ? 'w-0 overflow-hidden opacity-0 p-0 m-0' : 'lg:col-span-1 w-full opacity-100'} transition-all duration-300 space-y-4`}>
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                             <button
                                onClick={() => {
                                    // Save logic is implicit via state, but we can trigger a visual confirmation
                                    // or actually save to a 'Drafts' list if we had one implemented in storageService completely
                             ...
                             <div className="px-4 py-3 border-b border-slate-200 flex justify-between items-center bg-slate-50/50">
                                ...
                                <div className="flex gap-2">
                                     <button 
                                        ...
                                        className="text-xs bg-white border border-slate-200 text-slate-700 font-bold px-3 py-1.5 rounded hover:bg-slate-50 transition shadow-sm flex items-center gap-1"
                                     >
                                     ...
                                     <button 
                                        ...
                                        onClick={() => {
                                            // Toggle just one column for demo
                                    ...
                                    <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200 sticky top-0 z-20 shadow-sm">
                                        <tr>
                                            {tableConfig.filter(c => c.visible).map((col) => (
                                                <th key={col.id} className={`px-3 py-3 border-b border-slate-200 bg-slate-50 border-r border-slate-200/60 last:border-r-0 text-center ${getWidthClass(col.width)}`}>
                                                    {col.label}
                                                </th>
                                            ))}
                                            <th className="px-2 py-3 text-center w-12 bg-white border-b border-slate-200 sticky right-0 shadow-[-5px_0_10px_-5px_rgba(0,0,0,0.05)] border-l border-slate-100 z-30">
                                        ...
                                        {rfq.line_items.map((item, index) => (
                                            <tr key={item.item_id} className="hover:bg-blue-50/10 transition-colors group">
                                                {tableConfig.filter(c => c.visible).map(col => {
                                                    ...
                                                    // Input Box Style (Distinct Edge)
                                                    const inputClass = "w-full bg-slate-50 border border-slate-200 rounded-md px-2 py-1.5 text-xs text-slate-700 placeholder-slate-400 focus:bg-white focus:border-accent focus:ring-1 focus:ring-accent/20 focus:outline-none transition-all shadow-sm";
                                                    ...
                                                    if (col.id === 'shape') return <td key={col.id} className={`${cellClass} w-32`}><input value={item.product_type || ''} onChange={(e) => handleUpdateLineItem(index, 'product_type', e.target.value)} className={inputClass} placeholder="Shape" /></td>;
                                                    ...
                                                    if (col.id === 'tolerance') return <td key={col.id} className={`${cellClass} w-24`}><input value={item.tolerance || ''} onChange={(e) => handleUpdateLineItem(index, 'tolerance', e.target.value)} placeholder="-" className={`${inputClass} text-center`} /></td>;
                                                    
                                                    if (col.id === 'tests') return <td key={col.id} className={`${cellClass} w-24`}><input value={item.test_reqs?.join(', ') || ''} onChange={(e) => handleUpdateLineItem(index, 'test_reqs', e.target.value.split(',').map(s => s.trim()))} placeholder="-" className={inputClass} /></td>;
                                                    ...
                                                    if (col.id === 'wt') return <td key={col.id} className={`${cellClass} w-24`}><div className="flex items-center gap-1"><input type="number" value={item.size.wall_thickness.value || ''} onChange={(e) => handleUpdateDimension(index, 'wall_thickness', 'value', Number(e.target.value))} className={`${inputClass} text-right`} /><span className="text-[9px] text-slate-400 font-medium shrink-0">{item.size.wall_thickness.unit}</span></div></td>;
                                                    
                                                    if (col.id === 'length') return <td key={col.id} className={`${cellClass} w-24`}><div className="flex items-center gap-1"><input type="number" value={item.size.length.value || ''} onChange={(e) => handleUpdateDimension(index, 'length', 'value', Number(e.target.value))} className={`${inputClass} text-right`} /><span className="text-[9px] text-slate-400 font-medium shrink-0">{item.size.length.unit}</span></div></td>;
                                                    
                                                    if (col.id === 'qty') return <td key={col.id} className={`${cellClass} w-24`}><input type="number" value={item.quantity || 0} onChange={(e) => handleUpdateLineItem(index, 'quantity', Number(e.target.value))} className={`${inputClass} text-right font-bold text-slate-800`} /></td>;
                                                    
                                                    if (col.id === 'uom') return <td key={col.id} className={`${cellClass} w-24`}><input value={item.uom || ''} onChange={(e) => handleUpdateLineItem(index, 'uom', e.target.value)} className={`${inputClass} text-center text-slate-500`} /></td>;
                                                ...
                                                <td className="px-2 py-2 text-center w-12 sticky right-0 bg-white group-hover:bg-slate-50 transition-colors shadow-[-5px_0_10px_-5px_rgba(0,0,0,0.05)] align-middle z-10 border-l border-slate-100">
                                                    <button 
